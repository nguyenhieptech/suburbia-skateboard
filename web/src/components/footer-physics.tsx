...
const boardTextureURLs = [
  publicAssets.complete.completeBlackYellow,
  publicAssets.complete.completeGrayBlack,
  publicAssets.complete.completePinkDrop,
  publicAssets.complete.completeGreenNavy,
  publicAssets.complete.completeThankYou,
  publicAssets.complete.completeGridStreaks,
  publicAssets.complete.completeRedWhite,
];

export function FooterPhysics({ className }: { className?: string }) {
  const sceneRef = useRef<HTMLDivElement>(null);

  // Show fewer boards on mobile
  ...
  const limitedBoardTextures = isMobile ? boardTextureURLs.slice(0, 3) : boardTextureURLs;
  ...
  useEffect(() => {
    ...
    const render = Render.create({
      element: sceneRef.current, // attach to our scene div
  ...
  useEffect(() => {
    ...
    const boards = limitedBoardTextures.map((texture) => {
      ...
      return Bodies.rectangle(x, y, 100, 325, {
  ...
  return (
    <div ref={sceneRef} className={cn(className, "absolute inset-0 overflow-hidden")} />
  );
}
